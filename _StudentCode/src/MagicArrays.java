public class MagicArrays {
    ...
    public static boolean contains(String s) { // boolean for TRUE FALSE
        if (s == "Elrond") { // if the value is "Elrond"
            return true; //return true
        }
        return false; // otherwise, return false
    }// method contains

    public static boolean removeElement(int position) {
        // Make sure requested position is valid number
        if (position > -1 && position < a.length) {
            // Set up temporary array with room for one less element
            String[] temporary = new String[a.length-1];
            // everyone before the new addition maintains position
            for (int i = 0; i <= position; i++) {
                temporary[i] = a[i];
            }
            // everyone after the new deletion moves one position the left
        ...
        return false;
    } // method removeElement position

    public static boolean removeElement(String s) {
        return false;
    }// method removeElement String
